...
// Normally only works if the email was already added to the whitelist
// (by an existing admin). As a one-time bootstrap, if NO admin exists yet
// in the database and the submitted email matches FIRST_ADMIN_EMAIL in the
// environment, this route will auto-create that whitelist entry so you can
// set your first admin password without needing shell/DB access.
router.post('/signup', async (req, res) => {
  try {
    ...
    if (!admin) {
      ...
      if (firstAdminEmail && normalizedEmail === firstAdminEmail && !anyAdminExists) {
        admin = await AdminUser.create({ email: normalizedEmail, role: 'owner' });
      }
    }
